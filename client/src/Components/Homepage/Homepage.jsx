...
const Homepage = () => {
  // State to hold flat data
  const [flats, setFlats] = useState([]);

  // Fetch flat data from mockDataFlatDetails.json
  useEffect(() => {
    const fetchFlatData = async () => {
      try {
        const response = await fetch('/mockDataFlatDetails.json'); // Fetch from public folder
              ...
              <div key={flat.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  <div className="flex mt-2 space-x-2">
                    <MdWifi className="text-gray-500" />
                    <MdLocalParking className="text-gray-500" />
                    <MdHotTub className="text-gray-500" />
                  </div>
                </div>
              </div>
            ))}
